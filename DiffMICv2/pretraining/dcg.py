...
class DCG(nn.Module):
    def __init__(self, parameters):
        ...
        # construct networks
        # global network (now uses BottleneckKANHead via PostProcessingStandard)
        ...
        # MIL module (now uses BottleneckKANHead via AttentionModule)
        self.attention_module = m.AttentionModule(self.experiment_parameters, self)
        self.attention_module.add_layers()
        # fusion branch — not used (y_fusion = average of y_global + y_local)
    ...
    def forward(self, x_original):
        ...
        """
        # global network: x_original -> last_feature_map + class predictions via KAN
        h_g, self.saliency_map = self.global_network.forward(x_original)
        # saliency_map is now (B, num_classes) from BottleneckKANHead
        # y_global is the same as saliency_map (aggregation is identity now)
        self.y_global = self.aggregation_function.forward(self.saliency_map)

        # For ROI retrieval, we need a spatial attention map from the feature map
        # Use the last feature map h_g directly for crop localization
        # Generate a spatial saliency by averaging feature channels
        spatial_saliency = h_g.mean(dim=1, keepdim=True)  # (B, 1, H, W)
        # Resize to cam_size for ROI retrieval
        spatial_saliency_resized = F.interpolate(
            spatial_saliency, size=self.cam_size, mode='bilinear', align_corners=False
        )
        # Expand to num_classes channels for compatibility
        num_classes = self.experiment_parameters["num_classes"]
        spatial_saliency_cam = spatial_saliency_resized.expand(-1, num_classes, -1, -1)

        # region proposal network
        small_x_locations = self.retrieve_roi_crops.forward(x_original, self.cam_size, spatial_saliency_cam)
        ...
        # MIL module (classifier_linear is now BottleneckKANHead)
