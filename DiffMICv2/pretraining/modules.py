...
# ---------------------------------------------------------------------------
# BottleneckKANHead — replaces MLP guidance heads in DCG module
# ---------------------------------------------------------------------------
class BottleneckKANHead(nn.Module):
    """
    Replaces MLP guidance head in DCG module.
    Bottleneck projects high-dim SAM/encoder features to low-dim space
    before FastKAN to avoid curse of dimensionality.
    Safe for FP16 training on NVIDIA P100.
    """
    def __init__(self, input_dim: int, bottleneck_dim: int, output_dim: int):
        """
        Args:
            input_dim:      Dimension of incoming features
                            - Global head: match DiffMICv2 global encoder output dim
                            - Local head:  384 (EfficientSAM-ViT-S embedding dim)
            bottleneck_dim: Recommended 64. Must be <= 128. Do NOT increase beyond 128.
            output_dim:     Must match DiffMICv2's existing conditioning embedding dim
                            (keep same as the MLP head output_dim you are replacing)
        """
        super().__init__()
        # Step 1: Linear bottleneck — reduces dimensionality before KAN
        self.bottleneck = nn.Linear(input_dim, bottleneck_dim)
        self.bottleneck_norm = nn.LayerNorm(bottleneck_dim)

        # Step 2: FastKAN — learnable spline activations (RBF-based, P100-safe)
        # FastKAN([in, hidden, out]) — keep hidden = bottleneck_dim for simplicity
        self.kan = FastKAN([bottleneck_dim, bottleneck_dim, output_dim])

    def forward(self, x: torch.Tensor) -> torch.Tensor:
        # x shape: (B, input_dim)
        x = self.bottleneck(x)           # (B, input_dim) → (B, bottleneck_dim)
        x = self.bottleneck_norm(x)      # Normalize before KAN (stability on FP16)
        x = self.kan(x)                  # (B, bottleneck_dim) → (B, output_dim)
        return x
...
# ---------------------------------------------------------------------------
# PostProcessingStandard — now uses BottleneckKANHead (global guidance head)
# ---------------------------------------------------------------------------
class PostProcessingStandard(nn.Module):
    """
    Unit in Global Network that takes in x_out and produces class predictions.
    Replaced Conv2d with BottleneckKANHead: applies global avg pool first,
    then bottleneck + FastKAN.
    """
    def __init__(self, parameters):
        super(PostProcessingStandard, self).__init__()
        self.global_head = BottleneckKANHead(
            input_dim=parameters["post_processing_dim"],   # 512
            bottleneck_dim=64,
            output_dim=parameters["num_classes"]
        )

    def forward(self, x_out):
        # x_out: (B, C, H, W) — global feature map from ResNet
        # Global avg pool → (B, C)
        x = F.adaptive_avg_pool2d(x_out, (1, 1))
        x = torch.flatten(x, start_dim=1)
        # BottleneckKANHead → (B, num_classes)
        out = self.global_head(x)
        return out
...
class GlobalNetwork(AbstractMILUnit):
    ...
    def __init__(self, parameters, parent_module):
        ...
        if "use_v1_global" in parameters and parameters["use_v1_global"]:
            ...
            self.downsampling_branch = ResNetV2(input_channels=1, num_filters=16,
                     ...
                     growth_factor=2)
        # post-processing (now uses BottleneckKANHead)
    ...
    def forward(self, x):
        ...
        # feed into postprocessing network (now does GAP + KAN)
...
class TopTPercentAggregationFunction(AbstractMILUnit):
    ...
    def forward(self, cam):
        # cam is now (B, num_classes) from KAN head — already aggregated
...
# ---------------------------------------------------------------------------
# AttentionModule — now uses BottleneckKANHead (local guidance head)
# ---------------------------------------------------------------------------
class AttentionModule(AbstractMILUnit):
    ...
    def add_layers(self):
        ...
        # classifier — replaced with BottleneckKANHead (local guidance head)
        self.parent_module.classifier_linear = BottleneckKANHead(
            input_dim=512,                              # local ResNet-18 feature dim
            bottleneck_dim=64,                          # fixed
            output_dim=self.parameters["num_classes"]
        )
    ...
    def forward(self, h_crops):
        ...
        # map to the final layer (now uses BottleneckKANHead)
